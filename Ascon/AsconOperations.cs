...
namespace Ascon
{
    internal class AsconOperations
    {
        static void P_AddConstantLayer(ulong[] state, int i, int numberOfRounds)
        {
            state[2] ^= AsconConstants.RoundConstants[16 - numberOfRounds + i];
        }
        static void P_SubstutitionLayer(ulong[] state)
        {
            state[0] ^= state[4];
            state[2] ^= state[1];
            state[4] ^= state[3];
        ...
        static void Alternative_P_SubstutionLayer(ulong[] state)
        {
        ...
        static void P_LinearLayer(ulong[] state)
        {
        ...
        internal static void Permutation(ulong[] state, int i, int totalNumberOfRounds)
        {
            P_AddConstantLayer(state, i, totalNumberOfRounds);
            P_SubstutitionLayer(state);
            P_LinearLayer(state);
        }
        internal static ulong[] Pad(ulong[] data, int dataLength)
        {
            if (dataLength % 128 == 0)
                return data;
            else
        ...
        internal static void Initialization(ulong[] state, ulong[] key, ulong IV, ulong[] nonce)
        {
            state[0] = IV;
            state[1] = key[0];
            state[2] = key[1];
        ...
        internal static void ProcessAuthanticatedData(ulong[] state, ulong[] assocData)
        {
        ...
        internal static ulong[] FinalizationAndTagGeneration(ulong[] state, ulong[] key)
        {
            ...
            for (int i = 0; i < AsconConstants.NumberOfRounds_Pa; i++)
                AsconOperations.Permutation(state, i, AsconConstants.NumberOfRounds_Pa);
